...
__3.	What are some tools we can use to account for uncertainty in projections?__  
a.	“All subsets” model selection using AIC or another information criterion
__b.	Random draws from statistical distributions and replicated loops__ 
c.	A crystal ball and deck of tarot cards
d.	Simulate multiple different outcomes and use expert elicitation to determine which one is most likely  
...
res %>% 
  ...
  ungroup() %>% 
  group_by(type) %>% 
  summarize(`Average score` = round(mean(score, na.rm = T), 3))
```
  
Overall the assessment scores improved after the class, with the average increasing from 0.627 before the course to 0.712 after. The overall distribution of scores shifted higher.  
...
The proportion of correct answer improved for all questions.
  
  
#### Course evaluations  
  
We asked students to complete an anonymous course evaluation via Google Forms. Only 6 participants completed the survey.   
  
```{r}
eval_responses = gs_title("SSA 200 Course Evaluation - SAT Dec 2018 (Responses)")
...
eval %>% 
  select(-Timestamp) %>% 
  gather(question, response, c(1,3:8)) %>% 
  ...
  ggplot(aes(x = question, y = response, size = n)) +
